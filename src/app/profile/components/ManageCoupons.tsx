...
export default function ManageCoupons({ practitionerId }: ManageCouponsProps) {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      if (response.ok && result.success) {
        // Only show published and draft events
        const activeEvents = result.events.filter((e: Event) =>
  ...
  const handleSaveCoupon = async (couponData: Omit<Coupon, 'id' | 'usedCount'>) => {
    try {
      const apiData = {
        ...
        event_id: couponData.eventId ? parseInt(couponData.eventId) : null,
      };
...
function CouponModal({ coupon, onSave, onClose, events }: CouponModalProps) {
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              >
                <option value="">All Events (General Coupon)</option>
                {events.map((event) => (
              ...
              <p className="text-xs text-gray-500 mt-1">
                Select a specific event or leave as &quot;All Events&quot; to make this coupon available for any event
              </p>
            </div>
