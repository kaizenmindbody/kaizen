...
const ViewProfile: React.FC<ViewProfileProps> = ({ profile }) => {
  ...
  // Helper function to parse potentially JSON-stringified arrays
  const parseFieldValue = (value: any): string | string[] => {
    if (!value) return '';
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
          ...
          <div className="flex-1 text-center md:text-left">
            <h3 className="text-4xl font-bold text-gray-900 mb-2">
              ...
              {(() => {
                const parsedDegree = parseFieldValue(profile?.degree);
                if (!parsedDegree) return '';

                if (Array.isArray(parsedDegree)) {
                  return parsedDegree.length > 0 ? `, ${parsedDegree.join(', ')}` : '';
                }
                return parsedDegree.trim() ? `, ${parsedDegree}` : '';
              })()}
            </h3>
      ...
      {descriptionsData?.language && descriptionsData.language.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="flex flex-wrap gap-2">
            {(() => {
              const parsedLanguages = parseFieldValue(descriptionsData.language);
              const langArray = Array.isArray(parsedLanguages)
                ? parsedLanguages
